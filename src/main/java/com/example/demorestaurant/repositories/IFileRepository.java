...
public interface IFileRepository extends JpaRepository<Image, Long> {
    ...
    @Query(value = "select images.* from images " +
            "where images.restaurants_id = :restaurant_id and images.image_type = 'images'", nativeQuery = true)
    List<FileProjection> listAllImagesByRestaurantId(Long restaurant_id);

    @Query(value = "select images.* from images " +
            "where images.restaurants_id = :restaurant_id and images.image_type = 'logo'", nativeQuery = true)
    List<FileProjection> ListAllLogoImagesByRestaurantId(Long restaurant_id);

    @Query(value = "select images.* from images " +
            "where images.restaurants_id = :restaurant_id and images.image_type = 'banner'", nativeQuery = true)
